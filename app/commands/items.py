...
class ItemsCog(Cog):
    ...
    __item_quantity = Option(int, 'Choose a quantity to buy | any invalid value will be set to 1.', required=False)
    ...
    async def buy_item(self, ctx: ApplicationContext, item_id: int, item_quantity: __item_quantity):
        ...
        if item_quantity < 0:
            raise ValueError('Invalid value, quantity must be an integer above 0.')
    ...
    async def sell_item(self, ctx: ApplicationContext, item_id: int, item_quantity: __item_quantity):
        ...
        sell_item_result = bot_db.item_controller.remove_item(ctx.author.id, item_id, item_quantity)
        sell_order = embed_generator.simple_embed('Successfully sold!', sell_item_result)
        await ctx.respond(embed=sell_order)
